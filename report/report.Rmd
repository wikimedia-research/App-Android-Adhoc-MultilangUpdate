...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \caption{The feed can now include cards -- such as "Trending" (most read articles) -- in multiple languages.}
        \label{fig:feed}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
...
# Use latest customizations only:
card_language_combinations <- feed_customization %>%
  group_by(app_install_id) %>%
  top_n(1, client_dt) %>%
  ungroup %>%
  { purrr::map_df(.$enabled_list_v2, dplyr::as_data_frame) } %>%
  gather(card, language) %>%
